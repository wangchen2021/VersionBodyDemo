...
export default class DrawLineMidLeft extends AngleRender implements drawAngle {
    ...
    setAngle(angle: number) {
        ...
        this.startY = this.height * 0.36
        this.endX = this.width * 0.85
        this.endY = this.height * 0.36
        this.totalLength = this.endX - this.startX
    ...
    drawValueShapBack() {
        ...
        ctx.arc(this.width / 2 - this.totalLength / 2 * this.progress, this.startY, 45 * rpx + this.flashLoop * 20 * rpx, 0, Math.PI * 2)
        ctx.fill()
        ctx.closePath()
        if (this.flashLoop <= 0) {
    ...
    private drawValueShap() {
        ...
        ctx.arc(this.width / 2 - this.totalLength / 2 * this.progress, this.startY, 35 * rpx, 0, Math.PI * 2)
        ctx.fill()
        ctx.closePath()
    }
    private drawValueShapArrow() {
        drawArrow(this.pointer, this.ctx, this.width / 2 - this.totalLength / 2 * this.progress, this.startY - 17.5 * rpx, degreesToRadians(this.angle) + Math.PI / 2)
    }
