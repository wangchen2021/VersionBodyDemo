...
export default class DrawLoopMidRight extends AngleRender implements drawAngle {
    private redis = 250 * rpx
    private centerX = 380 * rpx
    private centerY = 500 * rpx
    private startAngle = Math.PI * 3 / 2
    private endAngle = Math.PI * 2
    ...
    sampleAngle(angle: number) {
        if (this.angle <= 0.15 * this.maxValue) {
            this.sampleRes = []
            return
        }
